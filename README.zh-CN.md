...
然后在 VS Code 中安装生成的 `vscode-json-viewer-0.1.1.vsix`（文件名以实际输出为准），或执行：
...
本地发布目标保持 `100 MB/s` 以上；由于共享 CI runner 的磁盘和 CPU 会抖动，CI 使用 `70 MB/s` 的保守下限，同时仍检查首屏、过滤延迟、取消响应、内存和 JSON 根节点上限。
